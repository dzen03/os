...
void bd_print() {
  for (int k = 0; k < nsizes; k++) {
    ...
    printf("  alloc:");
    bd_print_vector(bd_sizes[k].alloc, NBLK(k)); // TODO fix this
    if (k > 0) {
      printf("  split:");
...
void bd_free(void *p) {
  ...
  for (k = size(p); k < MAXSIZE; k++) {
    ...
    int need_to_merge = bit_isset(bd_sizes[k].alloc, buddy / 2);
    bit_invert(bd_sizes[k].alloc, bi / 2);      // free p at size k
    if (!need_to_merge) {                       // is buddy allocated?
...
int bd_initfree_pair(int k, int bi, int is_right) {
  ...
  if (bit_isset(bd_sizes[k].alloc, bi / 2)) {
    ...
    free = BLK_SIZE(k);
    if (buddy < bi)
      lst_push(&bd_sizes[k].free, addr(k, (is_right ? buddy : bi)));
    else
      lst_push(&bd_sizes[k].free, addr(k, (!is_right ? buddy : bi)));
  }
  return free;
...
int bd_initfree(void *bd_left, void *bd_right) {
  ...
  for (int k = 0; k < MAXSIZE; k++) {  // skip max size
    int left = blk_index_next(k, bd_left); // TODO fix
...
void bd_init(void *base, void *end) {
  ...
  for (int k = 0; k < nsizes; k++) {
    ...
    sz = sizeof(char) * ROUNDUP(NBLK(k) / 2 + NBLK(k) % 2, 8) / 8;
    bd_sizes[k].alloc = p;
